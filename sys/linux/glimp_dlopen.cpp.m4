...
dnl =====================================================
errprint(`m4 looking for include at: ' __file__ '\n')
include(`../gllog/gl_def.m4')

errprint(`gl_start = 'gl_start', gl_end = 'gl_end'\n')
errprint(`glX_start = 'glX_start', glX_end = 'glX_end'\n')
...
ifdef(`gl_start', `dnl gl_start is defined: gl_start=`gl_start''', `errprint(`ERROR: gl_start is NOT defined\n')m4exit(1)')
ifdef(`gl_end', `dnl gl_end is defined: gl_end=`gl_end''', `errprint(`ERROR: gl_end is NOT defined\n')m4exit(1)')
...
errprint(`Generating assign_funcptr for gl functions\n')
...
define(`safe_dlsym_funcptr', `dlsym_funcptr(`$1', `$2', `$3') if (!dll`$2') { GLimp_dlsym_failed("gl`$2'"); return false; }')
